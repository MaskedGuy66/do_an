...
def run_scoring(application_id: int, db: Session) -> None:
    ...
    try:
        ...
        if not app_record:
            logger.error(f"[BACKGROUND] Scoring thất bại: Không tìm thấy Application {application_id}")
        ...
        if not job:
            logger.error(f"[BACKGROUND] Scoring thất bại: Không tìm thấy Job {app_record.job_id} cho Application {application_id}")
            logger.error(f"Scoring failed: Job {app_record.job_id} not found for application {application_id}")
            return

        logger.info(f"[BACKGROUND] Đọc CV từ file: {app_record.cv_file_path}")
        # Extract CV text
        ...
        if not cv_text or len(cv_text.strip()) < 50:
            logger.warning(f"[BACKGROUND] Cảnh báo: CV text quá ngắn hoặc rỗng cho Application {application_id}. Hủy scoring.")
            logger.warning(f"Insufficient CV text for application {application_id}")
            return

        logger.info(f"[BACKGROUND] Đọc CV xong. Độ dài text: {len(cv_text)} ký tự. Preview: {cv_text[:200].strip()!r}")

        # JD criteria
        logger.info(f"[BACKGROUND] Bắt đầu đọc JD cho Job: '{job.title}' (ID: {job.id})")
        jd_text = f"Title: {job.title}\nDescription: {job.description or ''}\nRequirements: {job.requirements or ''}"
        jd_criteria = gemini_service.extract_jd_criteria(jd_text)
        logger.info(f"[BACKGROUND] Đọc JD xong. Kết quả trích xuất: required_skills={jd_criteria.required_skills}, min_years={jd_criteria.min_years_experience}")

        # Keyword Matching
        logger.info(f"[BACKGROUND] Bắt đầu Matching Word giữa CV và JD...")
        pre_match = gemini_service.pre_match_cv_with_jd(cv_text, jd_criteria.model_dump())
        logger.info(f"[BACKGROUND] Matching Word hoàn tất. Ngành: {pre_match['industry']} | Khớp: {pre_match['matched_required_skills']} | Thiếu: {pre_match['missing_required_skills']} | Năm KN: {pre_match['detected_years_experience']}")

        # AI Evaluation
        logger.info(f"[BACKGROUND] AI đánh giá bắt đầu cho Application {application_id}...")
        evaluation = gemini_service.evaluate_cv_against_jd(cv_text, jd_criteria.model_dump())
        logger.info(f"[BACKGROUND] AI đánh giá kết thúc cho Application {application_id}. Score: {evaluation.total_score} | Fit: {evaluation.fit_status}")
        ...
        db.commit()
        logger.info(f"[BACKGROUND] ===== Scoring hoàn tất cho Application {application_id}. Score: {evaluation.total_score} =====")
        logger.info(f"Scoring completed for application {application_id}: score {evaluation.total_score}")
    except Exception as e:
        db.rollback()
        import traceback
        logger.error(f"[BACKGROUND] LỖI khi scoring Application {application_id}: {str(e)}")
        logger.error(f"[BACKGROUND] Traceback:\n{traceback.format_exc()}")
        # Re-raise để background task runner ghi nhận thất bại
        raise
